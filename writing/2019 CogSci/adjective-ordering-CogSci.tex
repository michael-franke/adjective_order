...
Following \citeA{simonic2018} and \citeA{scontrasetalSPadjectives} we further assume that iterated adjectival modification triggers \emph{sequentially intersective context updates}. Later adjectives (syntactically further removed from the head noun) are interpreted relative to contexts which have already been restricted by previous adjectives. For example, the denotation of the phrase ``[adj\textsubscript{i} [adj\textsubscript{j} $N$]]'' given a shared context $C$ of potential referents is:
\begin{align}
...
\item A \emph{semantic threshold} $\theta \sim \mathcal{U}(0,1)$ is sampled
  ...
  threshold semantics in \eqref{eq:Semantics} of \citeA{schmidtetal2009} with
...
Based on $10^5$ Monte Carlo samples from the process outline above, we estimate the expected probability of recovering the speaker's intended referent based on the ordering ``big brown bag'' as \rlgetvariable{EU_big_brown}, compared to \rlgetvariable{EU_brown_big} for the ordering ``brown big bag''. The obtained samples of expected utilities for each ordering appear to be different indeed (paired $t$-test, $t \approx \rlgetvariable{Tstatistic}$, $p < 10e^{80}$). This lends credence to the general idea that, on average, the ordering of adjectives by subjectivity does affect average referential success and that using the less subjective adjective early in sequential interpretation is communicatively beneficial.
...
We also investigated the results from the (unconstrained) Monte Carlo simulation to learn about systematic influences of the model's parameters on the obtained results. Towards this end, we used a linear regression model, regressing the difference in expected utility between ``big brown bag'' and ``brown big bag'' is each simulated context against factors $n_{\text{ob}}$ (number of objects in context), $\theta$ (semantic threshold), and $\sigma_{\text{diff}}$ (difference $\sigma_{\text{big}} - \sigma_{\text{brown}}$), as well as all of their interactions. We then used iterated deletion of factors, following model comparison based on AICs, to determine the simplest factor structure relevant for explaining the predicted measure. The resulting, simplified model contained the factors listed in Table~\ref{tab:RegressionResults}, which also shows its estimates. These results indicate that all main factors except $\sigma_{\text{diff}}$ seem to influence the difference in expected utilities. The more objects in context, the less difference we expect; similarly for higher values of semantic threshold $\theta$. The latter, as such, may seem surprising but it is attenuated by the significant interaction terms where higher values of $\theta$ make us predict higher differences in expected utility when both $n_{\text{onj}}$ and $\sigma_{\text{diff}}$ are also high. Evidence for the latter interaction can also be seen in Figure~\ref{fig:Showcase_examples} where the effect of increasing $\theta$ is most pronounced in the upper right quadrant where$\sigma_{\text{diff}}$ is highest. \mf{I don't think that any of this is particularly interesting; feels like doing homework for a bad class; does anyone see anything more interesting in these results? or should we skip it altogether?}
...
\begin{itemize}
\item how to interpret these results: not rational deliberation + context-based optimal decision, but rather evolutionarily evolved (soft) preference
\item interpretation of subjectivity: subjective representations of agents are not (necessarily) the same as the formal linguist's degrees of a property; for us just an abstract way of implementing divergences in truth-value assignments; stochastic misalignments (as modeled here) could come from perception in context but also different general dispositions to classify a particular object as ADJ when paired with random other objects
\item extension to other scale types (not necessarily)
\end{itemize}
