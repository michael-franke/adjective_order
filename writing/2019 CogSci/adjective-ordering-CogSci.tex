...
\citeA{simonic2018} systematically explored the idea that subjectivity-based ordering preferences arise under pressure from successful reference resolution. Towards this end, they model a speaker's probabilistic choice of utterance ---e.g., \emph{big brown bag} vs.~\emph{brown big bag}--- within the Rational Speech Act tradition \cite<e.g.,>{FrankeJager2015:Probabilistic-p,goodmanfrank2016}. The speaker assumes that the listener might have a different lexical meaning for each adjective. If $L_i^{S,C}$ is the speaker's lexical entry for adjective $i$ in context $C$, the speaker believes that the probability that the listener has a lexical entry $L_i^{L,C}$ is:
\begin{align*}
  P(L_i^{L,C} \mid L_i^{S,C}) \propto
  \begin{cases}
    1 & if L_i^{S,C} = L_i^{L,C} \\
    \epsilon_i & \text{otherwise}
  \end{cases}
\end{align*}
The more subjective the adjective $i$, the higher the error probability $\epsilon_i$. With these beliefs about lexical divergence, \citeauthor{simonic2018} shows that the ordering \emph{big brown bag} is a more rational choice of the speaker than \emph{brown big bag} in a wide range of randomly generated contexts, if we additionally assume sequential context narrowing. \mf{explain this notion}

%* Sequential context narrowing: \den{adj_1~adj0~noun}_L^C = \den{adj_1}_L^{C \cap \den{adj0~noun}^C_L}
%* The probability, that both speaker and listener share the same extension of an adjective $\den{adj}$ in a common context $C$ depends from the subjectivity score. In other words, the literal listener is modeled to assume, that his extension $\den{adj}_L$ of an adjective $adj$ in the context $C$ is either different from the one of the speaker and needs to be sampled from all possible extensions with the probability $\epsilon_{adj}$, or the same as the one of speaker with the probability $1-\epsilon_{adj}$.
%* looked at the actual communicative success (= probability of the listener selecting the intended referent eventually)
...
With this assumption, \citeauthor{scontrasetalSPadjectives} demonstrate how subjectivity-based ordering preferences can maximize the probability of correctly classifying the intended referent with a multi-adjective string. Since adjectives closer to the noun operate over a larger set of potential referents (in \emph{big brown box}, consider the number of boxes vs.~the number of brown boxes), we minimize error by having less subjective, more error-prone adjectives classify a larger set of potential referents---in other words, we minimize error by placing less subjective adjectives closer to the noun. As with \citeA{simonic2018}, however, this finding is not fully general. The authors explored 103,740 cases of multi-adjective modification and found that subjectivity-based ordering behaved as expected in 93\% of those cases.
