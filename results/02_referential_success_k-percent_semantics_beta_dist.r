...
# rbeta dist params (shape1=alpha=1, shape2=beta=5)
# agent representation still uses rnorm, true values use rbeta instead of rnorm
...
get_outcomes = function(iterations, n_obj = 6, sd_brown = 0.1, sd_tall = 0.1, theta = 0.5, alpha = 1, beta = 1) {
  outcomes = map_df(1:iterations, function(i, nobj = n_obj) {
    
    true_values = matrix(c(rbeta(n_obj, shape1 = alpha, shape2 = beta),
                           rbeta(n_obj, shape1 = alpha, shape2 = beta)), 
                         nrow = 2, byrow = TRUE)
